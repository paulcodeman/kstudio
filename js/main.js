function init() {
    ...
    document.querySelectorAll('#right-panel .tab-bar input[type="radio"]').forEach(function (radio) {
        radio.addEventListener('change', function () {
            ...
            if (panel) {
                document.querySelectorAll('#right-panel .tab-panel').forEach(function (p) {
    ...
    if (S.win) {
        try {
            ...
            console.error('refreshPOS error:', e);
        }
    }
...
function onRectSelectMove(e) {
    ...
    if (x < 0) {
        S.select_element_rect.style.left = (S.sel_rect_x + x) + 'px';
        x = -x;
    }
    if (y < 0) {
        S.select_element_rect.style.top = (S.sel_rect_y + y) + 'px';
        y = -y;
    }
...
function setupToolbar() {
    ...
    if (gridSelector) gridSelector.onchange = function () {
        changeSetkaOption(this);
    };
    ...
    document.addEventListener('keyup', function (e) {
        ...
        if (S.cmd_event_ctrl) {
            if (ev.keyCode === 67) {
                ...
                paste_element();
            } else if (ev.keyCode === 90 && !ev.shiftKey) {
                e.preventDefault();
                undo();
            } else if (ev.keyCode === 90 && ev.shiftKey) {
                e.preventDefault();
